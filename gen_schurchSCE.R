...
colnames(markerData)[stringr::str_detect(colnames(markerData)," - ")] = colnames(markerData)[stringr::str_detect(colnames(markerData)," - ")] %>%
...
tissueType = phenoData %>% 
  ...
  mutate(subSpot = ifelse(stringr::str_detect(imageID, "A"), 1, 2)) %>% 
  group_by(subject) %>% 
